...
stage('Certificate Check') {
    steps {
		...
		script {
            ...
            def response = bat (script: 'curl --location --request POST \'http://certification-cloudhub-api.us-e2.cloudhub.io/api/certficate\' --header "Content-Type:application/java" --header "fileName:${files[0].name}" --header "appName:test-project-101" --header "envName:Sandbox" --header "orgName:Mulesoft" --data-raw \'${certDetails}\'',returnStdout: true)
            echo response
